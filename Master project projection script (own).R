...
query.marks = subset(FindAllMarkers(query, only.pos = FALSE, min.pct = 0.25),
                     subset = p_val_adj < 0.05)

#Figure 6 (HD, top panels)
...
FeaturePlot(query, features="TNFRSF13B",pt.size=1,label=TRUE,
            order=TRUE,reduction="ref.umap",label.size = 7)+ggtitle("TACI")

FeaturePlot(query, features="TNFRSF13C",pt.size=1,label=TRUE,
            order=TRUE,reduction="ref.umap",label.size = 7)+ggtitle("BAFF-R")
