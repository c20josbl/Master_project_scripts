## Master's project ueRA1-3-T3Dr script (own)
...
#Bar chart with patient percentage in each cluster 
...
#Stacked barplot with VDJ per cluster 
...
#Donut chart with DEGs per cluster
...
ggplot(df,aes(ymax=ymax,ymin=ymin,xmax=4,
              xmin=3,fill=Cluster))+
  geom_rect()+coord_polar(theta="y")+xlim(c(0,4))+theme_void()+
  geom_text(x=2,aes(y=labelPosition,label=label),size=5,
            check_overlap = T)+
  theme(legend.title = element_text(size=15,face="bold"),
        legend.text=element_text(size=15))
...
plot_density(ueRA,features="TNFRSF17",size=1,slot="counts")

#IL-6 and TNF-alpha

VlnPlot(ueRA,features=c("IL6","TNF"),assay = "integrated")
...
#IL6R expression
plot_density(ueRA,features="IL6R",size=1)
VlnPlot(ueRA,features="IL6R")
